@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex rounded-md shadow-sm rtl:flex-row-reverse sss">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            {{-- @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-500 bg-white border border-gray-300 cursor-default relatives dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span>
                                    </span>
                                @else
                            ...
                            @foreach ($element as $page => $url)
                                ...
                                @elseif ($page === $paginator->currentPage() - 1 || $page === 1 || $page === $paginator->lastPage())
                                    {{-- First and Last Pages --}}
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 dark:hover:text-gray-300 dark:active:bg-gray-700 dark:focus:border-blue-800" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @elseif ($page === $paginator->currentPage() + 2 && $paginator->currentPage() < $paginator->lastPage() - 1)
                                    {{-- Ellipsis after the next page --}}
                                    <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-500 bg-white border border-gray-300 cursor-default relatives dark:bg-gray-800 dark:border-gray-600">&hellip;</span>
                                @elseif ($page === $paginator->currentPage() - 2 && $paginator->currentPage() > 2)
                                    {{-- Ellipsis --}}
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium leading-5 text-gray-500 bg-white border border-gray-300 cursor-default relatives dark:bg-gray-800 dark:border-gray-600">&hellip;</span>
                                    </span>
